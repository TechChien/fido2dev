...
namespace Fido2Net
{
    ...
    public sealed unsafe class FidoAssertion : IDisposable
    {
        ...
        public void SetAuthDataRaw(ReadOnlySpan<byte> authData, int index)
        {
            fixed (byte* authData_ = authData)
            {
                Native.fido_assert_set_authdata_raw(_native, (UIntPtr)index, authData_, (UIntPtr)authData.Length).Check();
            }
        }
