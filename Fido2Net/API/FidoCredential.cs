...
namespace Fido2Net
{
    ...
    public sealed unsafe class FidoCredential : IDisposable
    {
        ...
        public ReadOnlySpan<byte> AAGUID
        {
            get
            {
                var len = (int)Native.fido_cred_aaguid_len(_native);
                var ptr = Native.fido_cred_aaguid_ptr(_native);
                return new ReadOnlySpan<byte>(ptr, len);
            }
        }

        public uint SigCounter
        {
            get
            {
                return Native.fido_cred_sigcount(_native);
            }
        }
        ...
        public void SetAuthDataRaw(ReadOnlySpan<byte> data)
        {
            fixed (byte* data_ = data)
            {
                Native.fido_cred_set_authdata_raw(_native, data_, (UIntPtr)data.Length);
            }
        }
